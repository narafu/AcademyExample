...
<%@page import="review.Member01"%>
<%@page import="review.MemberService01"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<%
	MemberService01 service = new MemberService01();
List<Member01> list = service.getMemberList();
%>
					...
					<table class="table">
						...
						<tbody>
							...
							for (Member01 m : list) {
								String style = "";
								if (index % 2 == 0)
									style = "style=\"background:yellowgreen\"";
							%>
							<tr <%=style%>>
								<td><%=++index%></td>
								<td class="title indent text-align-left"><a
									href="detail.html"><%=m.getBoss_id()%></a></td>
								<td><%=m.getId()%></td>
								<td><%=m.getBirthday()%></td>
								<td><%=m.getName()%></td>
							</tr>
